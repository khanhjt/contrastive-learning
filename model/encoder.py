...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.mean_fc = nn.Linear(config.vocab_size, config.num_topics)
    ...
    def forward(self, x):
        ...
        mean_posterior = self.mean_bn(self.mean_fc(x))
        log_var_posterior = self.log_var_bn(self.log_var_fc(x))
